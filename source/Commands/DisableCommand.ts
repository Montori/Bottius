...
export class DisableCommand extends AbstractCommand {
    ...
    public async runInternal(bot: Client, message: Message, messageArray: Array<string>) {
      ...
      const commandToDisable: string = messageArray[0];

      if (!messageArray.length) return super.sendHelp(message);
      if (!(CommandService.getInstance().getCommandMap().has(messageArray[0]))) { return message.channel.send(super.getFailedEmbed().setDescription('Please specify a valid command!')); }
