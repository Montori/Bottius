...
export class NomicCommand extends AbstractCommand {
    ...
    public async runInternal(bot: Client, message: Message, messageArray: Array<string>) {
      ...
      if (messageArray[0] == 'remove') {
        ...
        const nomicChannelIDList: Array<string> = partition.getNoMicList();
        const nomicChannels: Array<TextChannel> = nomicChannelIDList.map((channelID) => message.guild.channels.resolve(channelID) as TextChannel);
        if (!nomicChannels.length) return message.channel.send(super.getSuccessEmbed('There are currently no nomic channels'));
        const embed: MessageEmbed = super.getSuccessEmbed('List of nomic channels').setDescription(nomicChannels.join('\n'));
