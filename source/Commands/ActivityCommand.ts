...
export class ActivityCommand extends AbstractCommand {
    ...
    public async runInternal(bot: Client, message: Message, messageArray: string[]) {
      const embed: MessageEmbed = new MessageEmbed().setColor('00ff00').setAuthor('Activity changed').setDescription(`${messageArray.join(' ')}`)
      ...
      const activity: string = messageArray.join(' ').substring(activityStatus >= 0 ? this.ACTIVITY_TYPE[activityStatus].length : 0);
