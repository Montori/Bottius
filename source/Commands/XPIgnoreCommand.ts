...
export class XPIgnoreCommand extends AbstractCommand {
    ...
    public async runInternal(bot: Client, message: Message, messageArray: Array<string>) {
      ...
      if (messageArray[0] == 'remove') {
        ...
        const xpignoreChannelIDList: Array<string> = partition.getXPIgnoreList();
        const xpignoreChannels: Array<TextChannel> = xpignoreChannelIDList.map((channelID) => message.guild.channels.resolve(channelID) as TextChannel);
        ...
        const embed: MessageEmbed = super.getSuccessEmbed("Channels that don't gain xp").setDescription(xpignoreChannels.join('\n'));
