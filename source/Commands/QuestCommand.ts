import {
  Client, Message, MessageEmbed, DiscordAPIError, GuildMember,
} from 'discord.js';
...
export class QuestCommand extends AbstractCommand {
    ...
    public async runInternal(bot: Client, message: Message, messageArray: string[]) {
      if (messageArray[0] == 'add') {
        ...
        const discordAssignees = message.mentions.members.array();
        ...
        const newQuest: Quest = await this.questService.addQuest(message.member, discordAssignees, messageArray.join(' '));

        await newQuest.save();

        const questEmbed: MessageEmbed = new MessageEmbed()
        ...
        const finishedQuest: Quest = await this.questService.getQuest(Number(messageArray[1]), message.guild);
