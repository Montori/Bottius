...
export class LyricCommand extends AbstractCommand {
    ...
    public async runInternal(bot: Client, message: Message, messageArray: string[]) {
      if (messageArray[0] == 'add') {
        ...
        const artist = messageArray[1];
        messageArray = messageArray.slice(2);
        const songtext = messageArray.join(' ');

        const lyrics: Lyrics = await this.lyricService.addLyrics(artist, songtext, message.guild);
        ...
        const removedLyrics: Lyrics = await this.lyricService.removeLyrics(Number(messageArray[1]));
        ...
        const lyric: Lyrics = await this.lyricService.getRandomLyrics();
        ...
        const lyricEmbed: MessageEmbed = new MessageEmbed()
