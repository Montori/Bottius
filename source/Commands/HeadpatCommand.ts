...
export class HeadpatCommand extends AbstractCommand {
    ...
    public async runInternal(bot: Client, message: Message, messageArray: string[]) {
      const targetUser: GuildMember = message.mentions.members.first();
      ...
      if (targetUser) {
        ...
        const userToHeadpat: User = await this.userService.getUser(message.mentions.members.first(), message.guild);
