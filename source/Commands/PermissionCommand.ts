...
export class PermissionCommand extends AbstractCommand {
    ...
    public async runInternal(bot: Client, message: Message, messageArray: string[]) {
      ...
      const recMember: GuildMember = message.mentions.members.first();
      ...
      const permissionLevel: PermissionLevel = PermissionLevel[messageArray[2].toLowerCase()];
      ...
      const executingUser: User = await this.userService.getUser(message.member, message.guild);
      const receivingUser: User = await this.userService.getUser(recMember, message.guild);
