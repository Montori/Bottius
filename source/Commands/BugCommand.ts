...
export class BugCommand extends AbstractCommand {
    ...
    public async runInternal(bot: Client, message: Message, messageArray: Array<string>) {
      if (messageArray[0] == 'report') {
        ...
        const newBug: Bug = await this.bugService.addBug(message.member, messageArray.join(' '));

        await newBug.save();

        const BugEmbed: MessageEmbed = new MessageEmbed()
        ...
        const removeBug: Bug = await this.bugService.getBug(Number(messageArray[1]));
        ...
        const allBugs: Array<Bug> = await this.bugService.getAllBugs(message.guild);
