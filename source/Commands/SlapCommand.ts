...
export class SlapCommand extends AbstractCommand {
    ...
    public async runInternal(bot: Client, message: Message, messageArray: string[]) {
      ...
      const targetMember: GuildMember = message.mentions.members.first();
      if (targetMember) {
        if (this.cooldownService.isCooldown(message.member, `${this.commandOptions.commandName}<functional`)) { return message.channel.send(new MessageEmbed().setColor('#ff0000').setDescription(randomCooldownMessage[Math.floor(Math.random() * 2)])); }
        ...
        const userToSlap: User = await this.userService.getUser(targetMember, message.guild);
        ...
        if (userToSlap.headPats === 0) {
          ...
          if (randomInt === 1) {
            if ((await this.userService.getUser(targetMember, message.guild)).permissionLevel >= PermissionLevel.master) {
              const userToSlap: User = await this.userService.getUser(message.member, message.guild);
