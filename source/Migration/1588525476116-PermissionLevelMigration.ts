...
export class PermissionLevelMigration1588525476116 implements MigrationInterface {
    name = 'PermissionLevelMigration1588525476116'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE "temporary_user" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "discordID" varchar NOT NULL DEFAULT (''), "addedDate" datetime NOT NULL, "lastMessage" datetime NOT NULL, "totalMessages" integer NOT NULL DEFAULT (0), "totalPings" integer NOT NULL DEFAULT (0), "headPats" integer NOT NULL DEFAULT (0), "xp" integer NOT NULL DEFAULT (0), "permissionLevel" varchar CHECK( permissionLevel IN ('0','1','2','3','4') ) NOT NULL DEFAULT (0), CONSTRAINT "UQ_43c0d04d88a2beec7ed8431d7f1" UNIQUE ("discordID"))`, undefined);
        ...
        {
            await queryRunner.query(`INSERT INTO "user" (discordID, addedDate, lastMessage, totalMessages, totalPings, headPats, xp, permissionLevel) VALUES ('${master}', '${new Date()}', '${new Date()}', 0,0,0,0,4) ON CONFLICT (discordID) DO UPDATE SET permissionLevel=4`, undefined);
        }
    }
