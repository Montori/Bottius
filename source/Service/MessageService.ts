import {
  Client, Message, Role, MessageEmbed, TextChannel,
} from 'discord.js';
...
export class MessageService {
    ...
    public async handleMessage(message: Message) {
      ...
      const { customPrefix } = await (await this.partitionService).getPartition(message.guild);
      const prefixInvalid: boolean = customPrefix == null;
      ...
      if (message.content.substring(0, prefixInvalid ? this.prefix.length : customPrefix.length) != (prefixInvalid ? this.prefix : customPrefix)) {
        ...
        const command: string = messageArgs[0].slice(prefixInvalid ? this.prefix.length : customPrefix.length);
        const commandArgs: Array<string> = messageArgs.slice(1);
    ...
    private reactToUWU(message: Message) {
      if (!this.cooldownService.isCooldown(message.member, 'uwu')) {
        const owoArray: Array<string> = ['owo', 'uwu', 'pwp', 'qwq', 'OwO', 'UwU', 'QwQ', 'PwP', 'TwT'];
