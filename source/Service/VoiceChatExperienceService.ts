import {
  Client, GuildMember, VoiceState, Guild, VoiceChannel,
} from 'discord.js';
...
export class VoiceChatExperienceService {
        ...
        if (userList.length >= 2) {
          for (const voiceMember of userList) {
            if (!voiceMember.isMuted) {
              const user: User = await this.userService.getUser(voiceMember.member, voiceMember.member.guild);
              user.vcxp += 10;
              await user.save();
            }
          }
    ...
    public handleVoiceStateEvent(oldState: VoiceState, newState: VoiceState) {
      ...
      {
        const userArray = this.getUserArray(newState.channel);
      ...
      {
        const userArray = this.getUserArray(oldState.channel).filter((voiceMember) => voiceMember.id != oldState.member.id);
      ...
      } else if ((!oldState.mute && newState.mute) || (!oldState.deaf && newState.mute)) // user mute or deaf
      {
        ...
        const userIndex: number = userArray.findIndex((voiceMember) => voiceMember.id == newState.member.id);
      ...
      {
        const oldUserArray = this.getUserArray(oldState.channel).filter((member) => member.id != oldState.member.id);
        const newUserArray = this.getUserArray(newState.channel);
