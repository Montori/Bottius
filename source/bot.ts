import {
  Client, MessageEmbed, TextChannel, Role,
} from 'discord.js';
...
bot.on('guildMemberRemove', async (member) => {
  const partition = await partitionService.getPartition(member.guild);
  if (partition.leaveMessageActive && partition.leaveChannel) {
    const channel: TextChannel = member.guild.channels.resolve(partition.leaveChannel) as TextChannel;
...
bot.on('guildMemberAdd', async (member) => {
  const partition = await partitionService.getPartition(member.guild);
  const autoRoleList: Array<Role> = (await (AutoRole.find({ where: { partition } }))).map((autoRole) => member.guild.roles.resolve(autoRole.roleID));
